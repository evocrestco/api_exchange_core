...
class APITokenRepository(BaseRepository[APIToken]):
    ...
    @tenant_aware
    def get_valid_token(self, operation: str = "api_call", buffer_minutes: int = 5) -> Optional[Tuple[str, str]]:
        """
        ...
        try:
            ...
            if self.session.bind.dialect.name == "postgresql":
                ...
                expired_tokens = self.session.query(APIToken).filter(
                    APIToken.id.in_([row.id for row in expired_ids])
                ).all()
            else:
                    ...
                    self.logger.info(
                        ...
                        extra={
                            ...
                            "attempt_count": self.session.execute(
                                text("SELECT attempt_count FROM token_coordination WHERE tenant_id = :tenant_id AND api_provider = :api_provider"),
    ...
    def cleanup_expired_coordination_locks(self) -> int:
        ...
        try:
            deleted_count = self.session.query(TokenCoordination).filter(
                and_(
    ...
    def get_coordination_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            active_locks = self.session.query(TokenCoordination).filter(
                and_(
                    TokenCoordination.tenant_id == tenant_id,
                    TokenCoordination.api_provider == self.api_provider,
                    TokenCoordination.expires_at > datetime.utcnow()
            ...
            total_attempts = self.session.query(
                func.sum(TokenCoordination.attempt_count)
            ).filter(
                and_(
                    TokenCoordination.tenant_id == tenant_id,
                    TokenCoordination.api_provider == self.api_provider
            ...
            max_attempts = self.session.query(
                func.max(TokenCoordination.attempt_count)
            ).filter(
                and_(
                    TokenCoordination.tenant_id == tenant_id,
                    TokenCoordination.api_provider == self.api_provider
