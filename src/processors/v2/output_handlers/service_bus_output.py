...
from .config import ServiceBusOutputHandlerConfig, OutputHandlerConfigFactory
...
class ServiceBusOutputHandler(OutputHandler):
    ...
    def __init__(self, destination: str, config: Optional[Union[Dict[str, Any], ServiceBusOutputHandlerConfig]] = None):
        """
        ...
        if isinstance(config, ServiceBusOutputHandlerConfig):
            ...
            if config:
                ...
                if "scheduled_enqueue_time" in config and "scheduled_enqueue_time_utc" not in config:
    ...
    def validate_configuration(self) -> bool:
        ...
        if self.destination_type not in ["queue", "topic"]:
            self.logger.error(f"Invalid destination_type: {self.destination_type}. Must be 'queue' or 'topic'")
            return False
    ...
    def _prepare_service_bus_message(self, message: "Message", result: "ProcessingResult") -> ServiceBusMessage:
        """
        ...
        try:
            ...
            if self.scheduled_enqueue_time:
                ...
                sb_message.scheduled_enqueue_time = datetime.fromisoformat(self.scheduled_enqueue_time)
