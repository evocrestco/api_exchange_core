...
class ServiceBusOutputHandlerConfig(OutputHandlerConfigBase):
    ...
    def __post_init__(self):
        ...
        if not self.connection_string:
            # Try multiple environment variable names
            for env_var in ["AZURE_SERVICE_BUS_CONNECTION_STRING", "AZURE_SERVICEBUS_CONNECTION_STRING", "ServiceBusConnectionString"]:
                conn_str = os.getenv(env_var)
                if conn_str:
                    self.connection_string = conn_str
                    logger.debug(f"Loaded connection string from {env_var}")
    ...
    def from_env_prefix(
        ...
        for field_name, env_suffix in env_mappings.items():
            ...
            if value is not None:
                ...
                if field_name in ["enabled", "auto_create_queue", "create_queue_if_not_exists", 
                                 "create_directories", "append_mode", "include_metrics"]:
                    # Boolean fields
                    config_dict[field_name] = value.lower() in ["true", "1", "yes", "on"]
                elif field_name in ["max_retries", "retry_backoff_seconds", "timeout_seconds",
                                   "message_ttl_seconds", "visibility_timeout_seconds",
...
class OutputHandlerConfigManager:
    ...
    def add_config(
        ...
        """
        handler_config = OutputHandlerConfigFactory.create_config(
            handler_type, destination, config
        )
        self._configs[name] = handler_config
        
        logger.debug(
            f"Added output handler config '{name}': "
            f"{handler_type} -> {destination}"
        )
    ...
    def load_from_dict(self, configs_dict: Dict[str, Dict[str, Any]]) -> None:
        ...
        for name, config_data in configs_dict.items():
            if "handler_type" not in config_data or "destination" not in config_data:
                logger.warning(
                    f"Skipping config '{name}': missing handler_type or destination"
                )
                continue
