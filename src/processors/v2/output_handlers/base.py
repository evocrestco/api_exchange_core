...
class OutputHandlerResult(BaseModel):
    ...
    completed_at: datetime = Field(
        default_factory=lambda: datetime.now(UTC), description="When execution completed"
    )
    ...
    retry_after_seconds: Optional[int] = Field(
        default=None, description="Suggested delay before retry"
    )
    ...
    def create_skipped(
        ...
        return cls(
            ...
            destination=destination,
            execution_duration_ms=execution_duration_ms,
            metadata={**(metadata or {}), "skip_reason": reason},
        )
...
class OutputHandler(ABC):
    ...
    def _create_skipped_result(
        self, execution_duration_ms: float, reason: str, metadata: Optional[Dict[str, Any]] = None
    ) -> OutputHandlerResult:
