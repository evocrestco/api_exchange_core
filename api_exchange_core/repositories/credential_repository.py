...
class CredentialRepository(BaseRepository[ExternalCredential]):
    ...
    def _validate_tenant_isolation(
        self, credential: ExternalCredential, expected_tenant_id: str
    ) -> None:
        """
        ...
        try:
            ...
            if not credential:
                raise CredentialNotFoundError(
                    f"No credential found for system '{system_name}' " f"in tenant '{tenant_id}'"
                )
    ...
    def list_credentials(self, include_expired: bool = False) -> List[ExternalCredential]:
        ...
        try:
            ...
            self.logger.debug(
                "Listing credentials",
                extra={"tenant_id": tenant_id, "include_expired": include_expired},
            )
