...
from ..context.service_decorators import handle_repository_errors
from ..context.tenant_context import TenantContext, tenant_aware
from ..exceptions import ErrorCode, ServiceError, TokenNotAvailableError
from ..repositories.api_token_repository import APITokenRepository
...
class APITokenService:
    ...
    def get_valid_token(self, operation: str = "api_call") -> Tuple[str, str]:
        ...
        try:
            token_result = self.token_repository.get_or_create_token_with_coordination(
                operation=operation, buffer_minutes=5, token_generator=self.token_generator
            )
    ...
    def store_token(
        ...
        token_id = self.token_repository.store_new_token(
            token=token, generated_by=generated_by, generation_context=generation_context
        )
    ...
    def get_token_statistics(self) -> Dict[str, Any]:
        ...
        self.logger.debug(
            ...
            extra={"api_provider": self.token_repository.api_provider, "tenant_id": tenant_id},
        )
